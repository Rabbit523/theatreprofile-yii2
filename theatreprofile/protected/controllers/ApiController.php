...
class ApiController extends Controller
{
	...
	{
		return 'list,view,authorize,generatekey, users, uservalidcheck';
	}
    ...
    public function actionUsers()
    {
		...
		$this->_sendResponse(200, jsonizenc(['users'=>$users]));
	}
	
	// UserValidCheck

	public function actionUserValidCheck() {
		if(!isset($_GET['activeKey'])) {
			$this->_sendResponse(500, 'Error: Parameter <b>key</b> is missing' );
		}
		$model=UserKeys::validateKey($_GET['key']);
		if ($model->ipAddress == $_GET['ipAddress'] && $model->userID == $GET_['userID']) {
			$this->_sendResponse(200, "true");
		} else {
			$this->_sendResponse(200, "false");
		}
	}
