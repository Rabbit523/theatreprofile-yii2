 ...
 * @property Companyownership[] $companyownerships
 * @property Companywatchlist[] $companywatchlists
...
class Company extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, companyName, descr', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'companyaddresses' => array(self::HAS_MANY, 'Companyaddress', 'companyID'),
			'companyownerships' => array(self::HAS_MANY, 'Companyownership', 'companyID'),
			'companywatchlists' => array(self::HAS_MANY, 'Companywatchlist', 'companyID'),
			'productioncompanycrews' => array(self::HAS_MANY, 'Productioncompanycrew', 'companyID', 'with'=>'production', 'order'=>'case (ifnull(productioncompanycrews.startDate,production.startDate) is null and ifnull(productioncompanycrews.endDate,production.endDate) is null) when 0 then case (ifnull(productioncompanycrews.startDate,production.startDate) is not null and ifnull(productioncompanycrews.endDate,production.endDate) is null) when 1 then 1 else 2 end else 3 end,ifnull(productioncompanycrews.endDate,production.endDate) desc,ifnull(productioncompanycrews.startDate,production.startDate) desc'),
		);
	}
	...
	/*
	 * Custom slug generator for company model
	 */
