 ...
 *
 * The followings are the available columns in table 'tbl_carousel':
 * @property integer $id
...
class Carousel extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('label, caption, imageURL, href, status', 'required'),
			array('status', 'numerical', 'integerOnly'=>true),
			array('label', 'length', 'max'=>100),
			array('caption, imageURL, href', 'length', 'max'=>300),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, label, caption, imageURL, href, status', 'safe', 'on'=>'search'),
		);
	}
